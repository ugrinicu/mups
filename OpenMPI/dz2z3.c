...
{
   vect_t* input = (vect_t*)inputBuffer;
   vect_t* output = (vect_t*)outputBuffer;
...
int main(int argc, char* argv[]) {
   ...
   int num = MPI_Comm_size(MPI_COMM_WORLD, &numberOfThreads);
   ...
   vect_t* forces_temp_recv; 
   forces_temp_recv = malloc((n)*sizeof(vect_t));

   MPI_Op MPI_VECT_SUM;
   MPI_Op_create(&my_sum_function, 0, &MPI_VECT_SUM);

   for (step = 1; step <= n_steps; step++) {
      t = step*delta_t;
      memset(forces, 0, n*sizeof(vect_t));
      memset(forces_temp, 0, n*sizeof(vect_t));
      memset(forces_temp_recv, 0, n*sizeof(vect_t));
         
      for (part = my_rank; part < n-1; part+=numberOfThreads)
         Compute_force(part, forces_temp, curr, n);

      MPI_Allreduce(forces_temp, forces_temp_recv, n*2,
         MPI_DOUBLE, MPI_VECT_SUM, MPI_COMM_WORLD);

      for(int i = 0; i<n; i++) {
         forces[i][X] += forces_temp_recv[i][X]; 
         forces[i][Y] += forces_temp_recv[i][Y];
      }
